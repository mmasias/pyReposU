...
export const syncRepoIfNeeded = async (
  ...
  if (syncingRepos.has(repoUrl)) {
    console.log(`[SYNC] Ya hay una sincronización en curso para ${repoUrl}, omitiendo... ⏳`);
    return;
  }

  syncingRepos.add(repoUrl); // Marcar como sincronizando
  ...
  try {
    ...
    if (!repo) {
      ...
      repo = await RepositoryModel.create({ url: repoUrl, owner, name });
    }
    ...
    //  Comprobamos si ya existe el último commit en DB y si tiene archivos asociados
    ...
    //  Solo sincroniza si NO hay archivos o si el hash está desactualizado
    if (shouldSyncCommits && (!lastCommit || !hasFiles || localHash !== remoteHash)) {
      console.log(`[SYNC]  Sincronización necesaria, ejecutando syncCommits...`);
      console.time(`[SYNC] syncCommits duración`);
      await syncCommits(repo, localPath, { syncDiffs, syncStats });
      console.timeEnd(`[SYNC] syncCommits duración`);
    } else {
      console.log(`[SYNC] ✅ Último commit ya sincronizado con archivos asociados (${branch})`);
    }
    ...
    syncingRepos.delete(repoUrl); // Liberar el bloqueo
  }
};
...
export const syncCommits = async (
  ...
  const rawCommits = await getCommits(repo.url);
